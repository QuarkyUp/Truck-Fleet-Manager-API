...
const nbChromosome = 15;        // total nb of chromosomes
...
const mutationOdd = 0.1;        // mutation odd
let finalScore = [];
let finalDistance = [];

// const depCity = city[Math.floor(Math.random() * city.length)].name;
// const arrCity = city[Math.floor(Math.random() * city.length)].name;
...
const algoGen = (depCity, arrCity) => {
  ...
  /*******************************************/
  const path = arrChromosomes[0];
  const pathScore = finalScore[0];
  const pathDistance = finalDistance[0];

  for (let i = 0; i < path.length; i++) {
    const idxCity = path[i];
    const cityName = city[idxCity].name;
    path[i] = cityName;
  }

  return { "path": path, "score": pathScore, "distance": pathDistance };
}


module.exports = { algoGen, addCity, getCities, getDistances };